...
12. **频率标注默认关闭**。
13. **三段 EQ** — 低/中/高为控制点，柱间**线性插值**（无硬切阶梯）；默认 1.0 平坦。
14. **N16R8** — `qio_opi` + 16MB；瀑布进 PSRAM；TFT SPI 60MHz；FFT hop≈30FPS。
15. **噪声门** — 启动静音校准 2.5s，按频段减 floor；dB 映射 -60…0；Attack0.5 / Release0.1。
